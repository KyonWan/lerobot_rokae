...
def _transform_to_quat_wfirst(T: np.ndarray) -> list[float]:
    """从齐次变换矩阵提取位置和旋转，转为逆解接口需要的 [x,y,z,qw,qx,qy,qz]（四元数 w first）。"""
    pos = T[:3, 3]
    R_mat = R.from_matrix(T[:3, :3])
    q = R_mat.as_quat()  # scipy 返回 [x, y, z, w]
...
class InverseKinematicsProcessor(ProcessorStep):
    ...
    _cart_pos_flan_in_base: np.ndarray | None = field(default=None, init=False, repr=False)  # 法兰相对于基的笛卡尔位置 [x, y, z, rx, ry, rz]
    ...
    def __call__(self, transition: Transition) -> Transition:
        ...
        cart_pos_real_end_in_base = np.array([obs[f"cart_pos{i}"] for i in range(6)], dtype=np.float64)
        # 统一处理 psi：非7轴时为 0.0
        ...
        q_solution, cart_pos_cmd_end_in_ref = self.process_coordinate_transform_and_ik(cart_vel, q_current, cart_pos_real_end_in_base, psi)
    ...
    def process_coordinate_transform_and_ik(self, cart_vel, q_current, cart_pos_real_end_in_base, psi=0.0):
        """
        处理坐标系变换和逆解。
        ...
        Args:
            ...
            q_current: 当前关节位置
            cart_pos_real_end_in_base: 笛卡尔位置（end相对于base），必须提供（观测中是 end_in_base）
            psi: psi值，7轴时为实际臂角，非7轴时为 0.0
        
        Returns:
            (q_solution, cart_pos_cmd_end_in_ref): 关节解和 end 相对于 ref 的位置（更新后）
        """
        
        # 计算 T_end_in_flan（end 相对于 flange）
        T_end_in_flan = pose_to_transform(self.tool_end_pos)  # end 相对于 flange
        ...
        if self._cart_pos_flan_in_base is None:
            ...
            self._cart_pos_flan_in_base = transform_to_pose(T_flan_in_base)
            
            # 对于7轴，使用从观测中获取的psi
            if self.joint_num == 7:
                self._elbow = float(psi)

        # T_flan_in_base 从当前 _cart_pos_flan_in_base 计算（会变化）
        T_flan_in_base = pose_to_transform(self._cart_pos_flan_in_base)

        # 从 T_flan_in_base 计算 T_end_in_ref: T_end_in_ref = T_base_in_ref @ T_flan_in_base @ T_end_in_flan
